...
let ParserService = class ParserService {
    ...
    async launchBrowser() {
        ...
        try {
            this.browser = await ps.default.launch({
                ...parser_config_1.ParserConfig.browserConfig,
                args: [...parser_config_1.ParserConfig.browserConfig.args, parser_config_1.ParserConfig.proxy()],
            });
        }
        catch (error) {
            await this.logger.set({
    ...
    async solveCaptcha(page) {
        const capMonster = capmonstercloud_client_1.CapMonsterCloudClientFactory.Create(new capmonstercloud_client_1.ClientOptions({
            clientKey: await this.config.get("CAPTCHA_API_TOKEN"),
        }));
    ...
    async getExchangeRates() {
        try {
            const url = this.config.get("EXCHANGE_RATE_URL");
